...
export interface PreparedRunAgent {
  role: ScoutAgentRole;
  mount: CodexMount;
...
function requirePreparedAgents(
  agents: Partial<Record<ScoutAgentRole, PreparedRunAgent>>,
  roles: readonly ScoutAgentRole[],
): Record<ScoutAgentRole, PreparedRunAgent> {
  const result: Partial<Record<ScoutAgentRole, PreparedRunAgent>> = {};
  for (const role of roles) {
    const agent = agents[role];
    if (!agent) {
      throw new Error(`Run preparation did not produce agent runtime: ${role}`);
    }
    result[role] = agent;
  }
  return result as Record<ScoutAgentRole, PreparedRunAgent>;
}
