...
const COORDINATOR_INSTRUCTIONS_FILE = "agents/coordinator.AGENTS.md";
...
export function agentInstructionAssetPaths(isWorker: boolean): string[] {
  return isWorker
    ...
    : [COMMON_INSTRUCTIONS_FILE, COORDINATOR_INSTRUCTIONS_FILE];
}
...
/** Reads global and Coordinator instructions from the prepared mount. */
export function readCoordinatorAgentInstructions(options: ScoutAgentOptions): string {
  return [
    readAgentInstructions(options),
    readFileSync(join(options.agentMount.mountRoot, COORDINATOR_INSTRUCTIONS_FILE), "utf8"),
  ].join("\n\n");
}
