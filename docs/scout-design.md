...
flowchart TB
  ...
  subgraph L3["执行计划制定层"]
    ...
    PlanAgents["Plan Builder / Reviewer"]
...
- 执行计划制定层：负责把 Scout Input、Runtime Context 引用、asset commit cursor 和计划约束转成可审核的 `RuntimeExecutionPlan` draft。
  ...
  - Plan Builder / Reviewer：分别负责创建计划和独立审核计划。
...
这条链路只表达数据如何从输入走向验证结果，不表达具体 schema。外部验证输入、项目源码引用和外部文档先由 Context Understanding Agent 整理成 Scout Input；Runtime Context 提供当前 asset commit cursor、运行输入状态和策略边界；计划制定层把 Scout Input、Runtime Context 引用、asset commit cursor 和计划约束组成 `PlanBuildContext`，再生成 `RuntimeExecutionPlan` draft，并通过质量门控形成 Approved Plan；Runtime 执行 Approved Plan，驱动 node / ReAct 执行和观察，沉淀 node / ReAct 级验证数据；run 结束后再聚合为 run 级验证数据，进入确认与交付链路。
...
- Context Understanding Agent：理解外部 docs、source refs 和代码图引用，形成语义切片、`ValidationFactBody` 和 Scout Input；源码语义优先通过 CodeGraph MCP 进入语义切片。
...
- PlanBuildContext：把 Scout Input、Runtime Context 引用、asset commit cursor 和计划约束组织成 Builder / Reviewer 使用的上下文。
...
- run 级验证数据：基于所有有效 node / ReAct 数据生成聚合解释、证据画像和历史匹配结果；其中语义解释和交付草稿由 Agent 在 Skills 指导下生成，Runtime 负责记录来源、证据引用和校验结果。
...
- **skills**：走内部过滤 + Agent 选择 + 渐进式 mount pipeline。Asset Store 先按 `phase`、`tags`、`devices` 降噪，Runtime 把候选 metadata 和 refs 交给 Agent；Agent 选择后，Asset Store 再把对应 skill 增量物化，并形成新的 Asset Commit。新暴露的 skill 从下一 turn 开始可用。
...
- **注册与构图**：注册通过检查的资产，生成 Resource Registry、skill phase / tag 派生索引，并构建 asset refs graph。这个 graph 是 registry 的一部分，不是单独的运行索引。

加载流程：

```text
scan assets/codex/manifest.json
-> parse Codex native asset sources and shell-tools.json
...
- Registry 不证明 MCP server 在线、shell tool 可执行、设备在线、账号可用、后端可达或工具调用会成功。
...
- **初始请求**：run start 时由 profile、task 和默认运行环境生成，通常全量包含 config、AGENTS.md、plugins、MCP server config、hooks 和 shell tools。初始 skill 候选只以 metadata 和 refs 形式交给 Runtime / Agent；只有 required skill 或 Agent 已选择的 skill 才进入 Mount Pipeline。
...
Approved Plan 执行期不能依赖会继续变化的 `current-mount`。执行层必须从 Approved Plan 锁定的 asset commit cursor 解析出对应 immutable `mountRoot`，并以该 `mountRoot` 作为 cwd / writable root 的资产边界；artifact root 仍使用 run 级稳定路径。
...
- **Thread preflight**：由 Runtime 在 `thread/start` 后执行，用于验证依赖 thread context 才能确认的状态，例如最终 MCP server status。Thread preflight 不参与 Asset Commit 形成，但失败时必须 block 后续 run 推进。

Asset Store 定义 preflight 项和通过标准；Runtime 在 thread 创建后提交 thread preflight 结果，或根据失败结果 block run。
...
Context Understanding 本身由 `ScoutAgent` 执行，但它不消费 Scout Input；它负责生成 Scout Input。run start 必须先完成初始 Asset Commit，暴露 `context_understanding` skill、CodeGraph MCP 和默认读取能力，再创建 Context Understanding Agent 读取外部 docs、代码图和源码引用，生成语义切片、`ValidationFactBody` 和 Scout Input。

启动顺序：

```text
assets rebuild
-> initial Asset Commit with context_understanding skill / CodeGraph MCP / default abilities
-> Context Understanding Agent reads external docs and source refs through allowed abilities
...
### 5.1 External Context Documents
...
- 必须在进入计划制定前形成清晰的 `Validation Goal`、`Scenario` 和 `Acceptance Criteria`；无法形成时，必须 blocked 或 prompt。
...
- 无法形成清晰语义切片或可观测验证触点时，必须 blocked 或 prompt。

### 5.3 ValidationFactBody
...
如果引用文档缺失、互相矛盾、目标不清或无法形成可观测验证触点，Agent 必须把问题交给 Runtime 进入 blocked 或 prompt 流程，不能自行编造验证口径。
...
- 引用文档缺失、矛盾或不可观测时，必须 blocked 或 prompt，不能编造验证口径。

### 5.4 External Workflow Skill
...
执行计划制定层负责使用第 6 章定义的 `ScoutAgent` 调用抽象，把 Scout Input、Runtime Context 引用、asset commit cursor 和计划约束转成可检查的 `RuntimeExecutionPlan` draft，并通过独立审核把 draft 修正到可进入后续执行的质量水平。

本层由两个 Agent role 协作完成：Plan Builder Agent 负责创建计划，Plan Reviewer Agent 负责独立审核计划。二者使用同一份 `PlanBuildContext`，但运行在不同 `ScoutAgent` thread 中；Reviewer 不继承 Builder 的 thread history，只读取 draft、trace 和同一份上下文，从而降低被 Builder 解释锚定的风险。

**职责摘要**

- 通过 Plan Builder Agent 将输入事实和验证方法论推导成 `RuntimeExecutionPlan` draft。
- 让每个行为 node 都能追溯到目标、场景、证据要求、资源边界和方法论依据。
- 通过 Plan Reviewer Agent 对 draft 做分层审核、语义纠错和修复要求输出。
...
`PlanBuildContext` 是计划制定层传给 Builder 和 Reviewer 的同一份上下文封装。它不是新的事实来源，也不复制 Runtime Context 状态；它只持有 Scout Input、Runtime Context 引用、asset commit cursor 和计划约束。
...
- PlanBuildContext 必须带 input refs、Runtime Context 引用、asset commit cursor 和必要 hash，保证 Builder 与 Reviewer 在同一轮审核中面对的是同一组上下文输入。
...
- Approved Plan 形成前，Builder / Reviewer 如果发现资产能力不足，可以请求 Asset Store 追加能力型资产并形成新的 Asset Commit。
- 新 Asset Commit 形成后，Runtime Context 当前 cursor 前移；下一次 Builder / Reviewer turn 必须显式带入更新后的 `PlanBuildContext`、新的 cursor 和 commit summary。
- Builder / Reviewer 可以继续使用原 `ScoutAgent` thread，但 thread history 不是权威状态；最新 cursor 才是本次审核的资产水位。
- Reviewer 必须基于最新 cursor 重新审查能力覆盖；旧 cursor 下的通过结论不能直接复用。
- Approved Plan 只能在最终 cursor 下形成，并锁定该 cursor。

### 7.3 Plan Builder Agent

Plan Builder Agent 是基于 `ScoutAgent` 创建的计划生成角色。它使用 `PlanBuildContext` 创建新的 `RuntimeExecutionPlan` draft，并输出 `Plan Build Trace`。
...
const planResult = await builder.turn({
...
- 输出 `RuntimeExecutionPlan` draft、Plan Build Trace、assumptions、uncertainties 和 prompt / blocked candidates。

Builder 约束：

- Builder 不审核自己的计划是否通过。
- Builder 不能把工具动作当成 node 主语义。
- Builder 不能新增无法通过 `PlanBuildContext.assetCommitCursor` 查询到、或不被 Policy State 允许的能力、资产或资源边界。
- Builder 不能把不可观测风险写成可判定结论。

### 7.4 Plan Creation Method

计划不是从自然语言目标直接生成的，也不是把目标改写成步骤清单。Plan Builder Agent 必须按下面的推导链创建行为 node：
...
第一层是目标和场景归一。Builder 必须先识别本次要证明或排除的目标，再理解目标发生的入口状态、触发动作、关键路径和环境条件。
...
第二层是方法论选择。Builder 必须从 Scout Internal Skills 中选择适合当前场景的验证方法论，并说明选择依据。
...
第三层是 node 压实。Builder 必须把行为阶段压实为可执行、可观察、可判定、可交付的 behavior nodes。
...
如果一个候选 node 无法回答这些问题，它不能被保留为 node。Plan Builder Agent 要么重新拆解它，要么把缺失输入上报为 prompt / blocked。
...
- **Missing Signal Policy**：证据缺失时是 retry、degraded、blocked、prompt 还是 insufficient evidence。
- **Evidence Conflict Policy**：多个证据源冲突时如何处理。

Deliverable 设计需要说明每个 node 的结果如何进入最终材料：

- 哪些 NodeResult 支撑 pass / fail / insufficient evidence。
...
计划制定的产出是 `RuntimeExecutionPlan` draft。Draft 是计划候选，不是可执行事实；它必须通过 Plan Reviewer Agent 和 Plan Manager 的质量门控后，才能进入后续执行。

Draft 至少包含：

- **Plan Identity**：planId、runId、scenarioSlug、planSchemaVersion、createdAt 和 builder metadata。identity 不等同于业务身份。
...
### 7.8 Plan Reviewer Agent

Plan Reviewer Agent 是基于 `ScoutAgent` 创建的独立计划审核角色。Reviewer 使用与 Builder 相同的 `PlanBuildContext`，并把 Builder 输出的 draft 与 Plan Build Trace 作为审核 subject。
...
- Reviewer 必须运行在独立 `ScoutAgent` thread 中，不继承 Builder 的 thread history。
- Reviewer 不能只检查 draft 是否自洽，必须把 draft 放回同一份 `PlanBuildContext` 中审查。
...
- assumptions、uncertainties、degradation 和 prompt / blocked candidates 是否被显式表达。
...
- 证据充分性：证据设计是否足以支持 pass / fail / insufficient evidence，而不是只支持解释。
- 风险诚实性：无法确认、无法观察或输入不足的部分是否进入 prompt / blocked / insufficient evidence。
...
- 工具覆盖不足、资源不可用或策略不允许时，是否进入 prompt / blocked / insufficient evidence。
...
Layer 3 不要求运行前万无一失。它只要求计划声明的能力、资产和边界来自当前 `PlanBuildContext`，并且无法覆盖的部分被诚实标记为 prompt、blocked 或 insufficient evidence。

### 7.10 Repair Loop

Repair loop 由 Builder 和 Reviewer 交替完成。Builder 生成或修复 draft，Reviewer 独立审核并输出结构化结论，直到 pass、需要人工澄清或无法修复。

```text
Plan Builder Agent creates draft
-> Plan Reviewer Agent reviews draft with layered review
-> review verdict + failed standards + required fixes
-> Plan Builder Agent revises draft
-> Plan Reviewer Agent re-checks
...
```text
Plan Reviewer Agent reports missing ability
...
-> Builder / Reviewer continue review in the next turn
```
...
- **requiredFixes**：Builder 必须修复的具体问题。
...
- 质量过程：Plan Reviewer Agent 的 critique、required fixes、修复记录和最终 gate 状态。
- 资产水位变化：plan build / review 期间发生的 asset commit cursor 前移、commit summary、触发原因和重新审核记录。
- 未解决问题：prompt、blocked、degraded 或人工确认需求。
...
- 每个 node 如何服务验证目标。
- 每个证据要求从哪里来。
- Plan Reviewer Agent 发现过什么问题，如何被修复。
- 还有哪些风险没有被计划隐藏。
...
- 不能把 Agent 自评当成交付结论。
- 高风险、不确定或证据不足部分必须进入 prompt / blocked / review。

### 7.13 Approved Plan

Approved Plan 是通过质量门控后的 `RuntimeExecutionPlan`。它不是另一份由 Agent 重新生成的计划，而是一个 draft 在 Plan Reviewer Agent 审核通过、Plan Manager gate 通过后形成的不可变执行输入。

Approved Plan 的形成条件：

- Builder 输出的 `RuntimeExecutionPlan` draft 满足 Draft Contract。
...
- **Plan Build Trace Ref**：Builder 的拆解依据、asset reads、skill reads 和修复历史。
- **Approved At / Approved By**：批准时间和批准来源，通常由 Plan Manager 写入。

约束：

- Runtime 执行层只能消费 Approved Plan，不能直接执行 Builder draft。
...
Step Executor 消费 Approved Plan，驱动 logical behavior node、ReAct node、deterministic node、aggregation node 或 confirmation gate，并消费 NodeResult contract。对需要 instance 执行的 node，Step Executor 调用 Resource Manager 完成 instance resource resolution、worker context、调度、监控和 fan-in，再把结构化 NodeResult 交还给 State Controller。
...
验证层位于 Runtime 执行结果和交付确认之间。Runtime 负责执行与落账，验证层负责证据抽取、匹配、解释和聚合；Agent 可以参与解释和诊断，但不能用自由文本结论替代结构化证据匹配。
...
Step 级验证数据应保留当前 step 的意图、实际观察、语义切片、关键信号、缺失信号、噪声信号、推理和置信度。它是最靠近事实的验证材料，后续 review、confirmation 和 writeback 应优先引用 step 级语义切片和证据匹配结果，而不是重新解释原始日志或截图。

### 9.5 Run 级聚合与解释

Run 级验证数据基于所有有效 NodeResult、EvidenceBundle、ExecutionFact 和 step 级验证数据生成。它可以包含聚合解释、证据画像、历史匹配结果、uncertainties、missing signals、diagnostic summary 和 review material。
