...
- **严禁合并完整规则正文**：对必须完整消费的 `AGENTS.md`、`*.AGENTS.md`、`SKILL.md` 或 Skill 附属规则文件，每个 Tool 响应只能承载一份文件的完整正文。
- **严禁合并独立内容读取**：每个 Tool 响应最多承载一个独立的正文、源码或代码关系查询结果；对同一来源进行范围限定或结果过滤仍属于一个结果。有界的发现、状态和元数据查询按下方规则处理。
- **严禁混合规则正文与辅助查询**：不得把完整规则正文与 `pwd`、`ls`、`scout-assets` 等元数据或状态查询结果拼接到同一个 Tool 响应中。
- **严禁无界读取可能超限的规则文件**：单个文件预计可能超过输出限制时，必须为调用链各层明确设置足够的输出预算，或直接按连续且不重叠的行区间分段读取。
- **严禁重复无界读取已截断文件**：输出已经截断时，只补读尚未可靠获得的区间直到文件结尾，不得再次执行相同的无界完整读取。
- **批量短查询必须有界**：只有输出量可预估且合并后明确有界的元数据、状态和短查询才允许批量执行。
...
- **严禁隐藏探测诊断**：检查路径、权限或 Tool 可用性时，不得使用 `2>/dev/null` 等重定向丢弃错误，也不得用后续管道掩盖目标命令的退出状态；必须保留原始退出码和诊断输出供 Scout Runtime 记录。
