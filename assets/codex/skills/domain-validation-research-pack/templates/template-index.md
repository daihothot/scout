...
| templates/code-evidence.md | code evidence 聚合模板。 | 是 | 登记 managed codebase、version、implementation claims，并聚合 `E-CODE-*` refs。 |
...
| templates/capability-evidence.md | Capability 知识明细 evidence 模板。 | 条件 | 相关 Capability 被选中并需要分配 `E-CAP-*` 时使用。 |
| templates/availability-evidence.md | 目标版本可用性聚合 evidence 模板。 | 是 | 每个 Research Pack 必须生成唯一的 `E-AVAIL-001`。 |
| templates/platform-evidence.md | 目标平台知识聚合 evidence 模板。 | 是 | 每个 Research Pack 必须生成唯一的 `E-PLATFORM-001`。 |

## Maintenance Rules

- 本索引只做导航。
- 模板 frontmatter 中的 `scout.resource` 只属于 Scout Runtime 的模板资源控制 metadata；生成的 Research artifact frontmatter 不得复制该字段。
...
- `E-CAP-*`、`E-AVAIL-001` 和 `E-PLATFORM-001` 的模板由本目录所有；知识 producer 只提供事实来源和 source refs。
