...
- Domain Skill 必须在 frontmatter 中定义非空 `phase` inline list。Coordinator Domain Skill 固定使用 `[Synthesis]`；Worker Domain Skill 使用所属 domain 声明的实际 Worker Phase。Domain Skill 不得使用 `Startup`。
