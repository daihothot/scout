...
| templates/research-index.md | 输出文件 `index.md` 使用的研究总览 artifact 模板。 | 是 | 记录 pack 状态、artifact refs、knowledge/root/source repo provenance 和 phase resume。 |
...
| templates/knowledge-evidence.md | knowledge evidence id 的摘要聚合模板。 | 是 | 不嵌入完整 evidence block；必须包含 11 个规格维度覆盖矩阵。 |
...
| templates/code-evidence.md | code evidence 聚合模板。 | 是 | 登记 implementation claims、root/source repo provenance，并聚合 `E-CG-*` 和 `E-CODE-*` refs。 |
| templates/evidence-registry.md | evidence id registry 模板。 | 是 | 索引全部 evidence ids 和 supports links。 |
| templates/verification-manual.md | verification manual 模板。 | 是 | 下游手册；Given/When/Then 只来自 BDD 和画像，Signals 只描述待采集数据。 |
