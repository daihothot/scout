...
summary: 定义按 BDD 与目标版本复用的 RBT Execution Pack、执行文件及其 handoff contract。
---
...
- 一个 Pack 只对应一个 BDD identity 和一个目标版本；同一组合复用同一 Pack 与执行文件，不按 Scout run 重复生成。
- 执行文件与 Pack 同时由 Executor 生成，保存从 `behavior.campaign.start` 到 `behavior.campaign.stop` 的简化 `command + payload` 序列。
- Pack 完整只表示 Agent 侧交付内容完整，不表示 BDD 已通过、失败或已被 Runtime 覆盖。
- BDD、代码、Journal expectation、Signal expectation 和 Human Input 是不同事实，必须由对应 artifact 分开保存。
- Pack 只保存 Agent 侧可定位事实、选择、预期、观察和 refs；执行文件只保存可重放命令序列。两者都不保存实际命令结果、trace、evidence、campaign journal 或 cleanup 正文。
...
2. 按 `template-index.md` 的 required 列表读取对应 templates，包括执行文件 contract；
3. 生成 `E-CODE-*` 时，再读取本 Skill 的 `templates/source-code-evidence.md`；
...
- 执行前计划、可重放执行序列、Journal expectation、Signal expectation；
- Human Input 记录和限制；
...
Pack 使用当前 Executor 私有 artifact root 下的 canonical 目录：
...
`execute-file.json` 和 `execute-pack/` 由 Executor 写入。同一 BDD 与目标版本原地更新，不创建副本。
...
- `execute-file.md` 定义独立可重放 `execute-file.json` contract；执行文件不保存实际执行事实。
...
- Pack、执行文件、Human Input 已解决或 requirement 已定义都不能解释为 BDD 已覆盖。
- `execute-file.json` 不重复保存 BDD ID 或目标版本；二者由 canonical path 唯一确定。
- 执行结果与预期不一致时记录事实和限制，不改写为通过，不因不匹配自动启动新的执行。

## Failure Rules

- 必需 artifact 缺失、来源不可读、ID 冲突、引用断裂或 required Signal expectation 未按 template 写入时，不能提交 `ready + complete`。
- 只有 `RequestHumanInput` 返回真实 request identity 后，才能建立 Human Input artifact；失败调用不算正式申请。
...
- 禁止把实际 command result、宿主 shell input/output、trace、evidence、campaign lifecycle 或 cleanup 正文复制进 Pack 或执行文件。
...
- Pack 只对应一个 BDD 和一个目标版本，目录和 `pack_id` 稳定。
- 执行文件与 Pack 同时生成，且只保存 artifact contract 与 `command + payload` 执行顺序。
- 所有 Agent 描述性内容为中文，技术原始值保持原样。
- 每个 artifact 使用对应 template 结构，状态、ID、refs、locators 和 supports 关系闭合。
- 已确认的代码、Journal、Signal 和 Human Input facts 没有混为一种事实。
- 实际命令、返回值、journal 和 live evidence 没有复制进 Pack。
